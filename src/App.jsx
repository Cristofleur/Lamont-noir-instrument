    ...
    <div className="dash-row">
      ...
      <span className="cf" style={{ fontSize:"14px", color:"#D8CCB8", textAlign:"right", lineHeight:1.4 }}>{value}</span>
        ...
        <Sec title="Pillar Distribution">
          {sorted.map(([key, val]) => {
            const p = PILLARS[key];
            const pct = ((val / 81) * 100).toFixed(1);
            return (
              <div key={key} style={{ marginBottom:"12px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"4px" }}>
                  <span className="sf" style={{ fontSize:"9px", color:"#D8CCB8" }}>{p.label} — {p.axis}</span>
                  <span className="sf" style={{ fontSize:"9px", color:"#6B5A4E" }}>{val} ({pct}%)</span>
                </div>
        ...
        <Sec title="Primary Diagnostics">
          <Row label="Primary Pillar" value={`${PILLARS[primary].label} (${PILLARS[primary].axis})`} />
          <Row label="Secondary Pillar" value={`${PILLARS[secondary].label} (${PILLARS[secondary].axis})`} />
        ...
        <Sec title="The Decree">
          <div className="decree-block">
            <p className="cf" style={{ fontSize:"15px", color:"#E8DFD0", lineHeight:1.6, fontStyle:"italic" }}>
              {decree}
            </p>
          </div>
        </Sec>
...
export default function App() {
  ...
  const handleBegin = () => setPhase("rules");
  const handleStartQuestions = () => { setQIdx(0); setPhase("transition"); };
  const handleTransitionEnd = () => setPhase("question");
  ...
  const handleContinue = () => {
    if (qIdx < QUESTIONS.length - 1) {
      const nextQ = qIdx + 1;
      setQIdx(nextQ);
      if (QUESTIONS[nextQ].s !== QUESTIONS[qIdx].s) {
        setPhase("transition");
      } else {
        setPhase("question");
      }
    } else {
      setScores(computeScore(answers));
      setPhase("completion");
    }
  };
  ...
  return (
    <>
      <GlobalStyles />
      {phase === "threshold" && <ThresholdPage onEnter={handleBegin} />}
      {phase === "rules" && <RulesPage onBegin={handleStartQuestions} />}
      {phase === "transition" && (
         <SectionTransitionPage
           section={SECTIONS.find(s => s.id === QUESTIONS[qIdx].s)}
           onContinue={handleTransitionEnd}
         />
      )}
      {phase === "question" && (
         <QuestionPage
      ...
      {phase === "completion" && <CompletionPage onHouseAccess={() => setPhase("dashboard")} />}
      {phase === "dashboard" && <DashboardPage scores={scores} />}
    </>
  );
}
